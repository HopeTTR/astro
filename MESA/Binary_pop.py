...
#-------------------------------------#
    ...
    # Try submitting the job using sbatch from inside the bin directory
    try:
        subprocess.run(["sbatch", "binary_job.sh"], cwd=bin_path, check=True)  # Submit the job via sbatch inside the binXX directory
        print(f" Submitted job for {bin_name}")  # Print success message
    except subprocess.CalledProcessError as e:
        print(f" Submission failed for {bin_name}: {e}")  # Print error message if sbatch fails
